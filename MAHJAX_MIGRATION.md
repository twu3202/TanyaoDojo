...
- **R4 评测桥(神圣不可变)**:mjai 协议适配器把 Mahjax 模型包成 mjai bot,接入既有 libriichi
  ...
  - **宽网试探(2026-08-02,bc_w192 = 192ch×8blk/5.3M 参数,六年池)**:4k 局 =
    **-10.53±2.71,与窄网 v3(-10.17)/v3.5(-10.46)统计等同——容量 ×3 未破平台**。
    两种解释待分辨(验证 epoch 带日志重训中):①欠训(重启丢日志,可能仅 1 epoch,
    val 未知);②**平台是观测信息瓶颈**(20 平面 vs Mortal 938:丢了牌河时序/摸切旗/
    立直宣言位置/宝牌时点等)→ 若 val 与窄网同为 ~80%,下一杠杆 = obs v2 加富
    (而非再加宽网络)。league 线不受影响(基座本就是窄网 v3)。
