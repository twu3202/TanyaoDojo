...
def collate(batch):
    ...
    # new pyo3 returns [u8; 4] as bytes; torch.tensor(bytes, pin_memory=True) misparses it.
    # No pinning here: collate runs in forked DataLoader workers, where touching
    # CUDA (pin_memory/is_available) raises "CUDA error: initialization error".
    rank_by_players = torch.tensor([list(r) for r in rank_by_players], dtype=torch.int64)
...
def train():
    ...
    # macOS: torch DataLoader workers segfault with the Rust extension; 0 = load in-process
    grp_num_workers = cfg['dataset'].get('num_workers', 1)
    train_data_loader = iter(DataLoader(
        dataset = train_file_data,
        batch_size = batch_size,
        drop_last = True,
        num_workers = grp_num_workers,
        collate_fn = collate,
    ))
