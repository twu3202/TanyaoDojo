...
**多跑的 1.9 亿步没买到任何东西**:b2272 − b800 配对差 = **−0.528 ± 0.486**(z=−2.13,
压在显著性边界上;按本项目的规矩压线不算数,但"没变强"是确定的)。学习在 1 亿步前
就停了,和熵塌到 0.048 nats 对得上。
...
**P3 因此改序**:原计划直接上 100 亿步 + league,但证据说这个配方在 1 亿步就停止学习,堆 100 倍算力只会把同一个塌缩策略跑得更久。开工前先提 `ent_coef`(0.01→0.03)从 b800 附近续训,用 `policy_shift.py` 那套先行指标确认策略真的重新开始移动,再谈对手池。

**同时必须记下的隐患**:该策略的熵已塌到 **0.048 nats**(≈1.05 个有效动作),
逐状态 max_ratio 冲到 3-6、max_kl 到 1.5-4;均值型指标(clip_frac 0.006、
adv_osc 0.02、vloss 在降)全都好看,**只有尾部仪表报警**——这就是把它们做成
免费常驻指标的理由。P2 的判据不看熵,但 P3 要对付的是会适应的对手池,
一个几乎确定的策略很可能撑不住,届时先提 `ent_coef` 再谈别的。
