...
| 线 | 状态 | 最优 | 备注 |
|---|---|---|---|
| **价值线**(Mortal 栈,在线值回归 + 锚定) | **跑中**(奖励用评测口径顺位点) | **-0.20 ± 1.33**(12k,408,000 步;配对 vs C' final +0.59,z=0.83,不显著) | 训练奖励 pts=[6,4,2,0] 与评测 [90,45,0,-135] 不一致是真头寸:改同比例后被飞率与 v4 持平、4 位率 pp 从 +0.87 降到 +0.25、顺位拆解里南四兑现从 -1.02 收到 -0.00;点估计为项目最好但配对不显著,448,000 步再定音 |
