...
| 线 | 状态 | 最优 | 备注 |
|---|---|---|---|
| **价值线**(Mortal 栈,在线值回归 + 锚定) | 四个杠杆 100k 全零,**转为先定位** | **-0.64 ± 0.46**(**100k**,408,000 步) | λ / 锚目标 / 奖励口径 / 对手池都只让打法在攻守等强度线上滑动(±25 点/局),100k 配对全不显著;缺口稳定落在被飞提前结束的对局(南四兑现 -1.1 ~ -1.3,±0.26)。正在用按分数分档的决策探针定位低分局面的行为差 |
