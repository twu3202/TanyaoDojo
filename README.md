...
| 线 | 状态 | 最优 | 备注 |
|---|---|---|---|
| **价值线**(Mortal 栈,在线值回归 + 锚定) | **跑中**(对手池换成 v4) | **-0.64 ± 0.46**(**100k**,奖励口径分支 408,000 步;448k 为 -0.93,同牌配对分不开) | 缺口集中在被飞提前结束的对局(100k 上 -1.1);奖励口径改动在 8k 上的机制改善 100k 上大部分不成立(已更正)。正在验证第二个训练/评测不一致:自对弈只有 17% 对局打 v4 |
