...
| 线 | 状态 | 最优 | 备注 |
|---|---|---|---|
| **价值线**(Mortal 栈,在线值回归 + 锚定) | 自建基座 100k **-0.64**;从 v4 出发微调 100k **+0.10 ± 0.34**(CI 含 0) | **+0.102 ± 0.337**(100k,基座 = 冠军 v4 本身,4 万步;零点校准 v4 自打 v4 = -0.056 ± 0.091) | 点估计达到用户判据(>= +0.01)但不显著;要把 +0.1 测显著需约 110 万局(约 55 GPU 小时)。机制:从 v4 出发后"被飞多 / 放铳贵"消失(+0.02pp vs 自建基座 +0.28pp),证实那是自建基座支系的属性 |
