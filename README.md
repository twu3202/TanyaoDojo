...
| 线 | 状态 | 最优 | 备注 |
|---|---|---|---|
| **价值线**(Mortal 栈,在线值回归 + 锚定) | **跑中**(对手池换成 v4) | **-0.64 ± 0.46**(**100k**,奖励口径分支 408,000 步) | 缺口集中在被飞提前结束的对局(100k 上 -1.1)。奖励口径改成评测比例:100k 配对 +0.25(z=1.01,不显著),顺位分布变温和但被飞亏损没动;正在验证对手池(自对弈原先只有 17% 打 v4) |
