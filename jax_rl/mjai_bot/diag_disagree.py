...
**已知局限**(均在输出中单独可见,不混进主结论):
  · **Q 差不是强度的代理 —— 已被天然反例证伪**(2026-09-11,同一条 v4 轨迹 4000 局):
    v11(100k −2.50)Q差/局 5.18,rl1_best(100k −1.96)6.31 —— **更强的那个离 v4 的偏好更远**。
    阳性对照验证的是"重放与编码正确",不是"v4 的 Q 能给偏离定价"。这张图只回答**在哪分道**,
    不回答**分道值多少**;拿 Q 差排优先级之前,必须先有复式强度读数撑腰;
...
def main():
    ...
    print(f"\n  合计 Q差/局 = {tot_gap:.4f}(被测偏离参考模型偏好的程度;**不是强度损失**,见文件头反例)")
    print(f"  合计 实际Q差/局 = {tot_lgap:.4f}(该座位实际打出的动作偏离 v4 偏好的程度,**不是强度损失**;"
