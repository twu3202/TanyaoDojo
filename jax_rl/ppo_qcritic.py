"""
Q-critic League PPO:中心化 Q + Expected SARSA(λ)(fork 自 ppo_oracle.py)。

与 ppo_oracle 的唯一算法差异(其余逐字一致,可做干净 A/B):
  critic 从 V(s) 换成 **Q(s,a | 全信息)**,时序差分的 bootstrap 值从
  "采样到的下一动作的值" 换成 **策略加权期望 V(s)=Σ_a π(a|s)Q(s,a)**。
动机(arXiv 2605.19235,MIT 2026):自博弈的均衡策略是随机的,GAE 的 multi-step
backup 沿采样轨迹回传,把"不同动作 Q 值本就不同"的差异当成噪声放大;换成期望
backup 后,斗地主上以 -40% 训练步数超过 PerfectDou。本项目 RL 四连负的形态
(内部指标漂亮、外部强度掉)正指向"优势信号→策略更新"这一段,故此为首选修法。

策略优势 = λ-回报 − 期望基线:A = G^λ(s,a) − Σ_a π(a|s)Q(s,a)。

--- 以下为继承自 ppo_oracle 的非对称 AC 说明 ---
...
class Transition(NamedTuple):
    ...
    action: jnp.ndarray
    value: jnp.ndarray            # Q(s, a_taken) —— 被执行动作的中心化 Q
    v_exp: jnp.ndarray            # Σ_a π(a|s)Q(s,a) —— 期望基线(Expected SARSA)
    reward: jnp.ndarray
...
def make_train(network: nn.Module, critic: nn.Module, magnet_params):
    ...
    def rollout(all_params, critic_params, assign, learner_seat, env_state, key, oracle_scale):
        def one_step(carry, _):
            ...
            q_all = critic.apply(critic_params, obs)              # (B, A) 1x 前向,非 K+1
            logits = jnp.where(mask, logits, NEG)
            dist = distrax.Categorical(logits=logits)
            action, log_prob = dist.sample_and_log_prob(seed=k_act)
            # 期望基线:非法动作的 π=0,其 Q 值不进期望
            v_exp = jnp.sum(dist.probs * q_all, axis=-1)          # (B,)
            value = jnp.take_along_axis(q_all, action[:, None], axis=-1).squeeze(-1)
            ...
            t = Transition(done, action, value, v_exp, reward, log_prob, obs, mask, cur,
                           learner_seat)
    ...
    # 逐座位链与 ppo_league 同构,但 bootstrap 值换成期望基线 v_exp(Expected SARSA)
    def calculate_gae(traj: Transition):
        def single_env(tr: Transition):
            def scan_fn(carry, t):
                ...
                new_carry = (gae, next_value.at[player].set(t.v_exp), racc,
    ...
    def process(traj: Transition):
        ...
        vm = vm & seat_onehot
        # 策略优势 = λ-回报 − 期望基线(而非 λ-回报 − Q(s,a_taken))
        v_exp_4 = jax.nn.one_hot(traj.current_player, NUM_PLAYERS) * traj.v_exp[..., None]
        adv = tgt - v_exp_4
        ...
        def epoch(carry, _):
            ...
            def mb_step(carry_in, mb):
                ...
                def critic_loss_fn(cparams):
                    q_all = critic.apply(cparams, tr.observation)          # (mb, A)
                    v = jnp.take_along_axis(q_all, tr.action[:, None], axis=-1)
                    v_old = tr.value[..., None]
